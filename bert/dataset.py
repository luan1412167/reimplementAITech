...
class IMDBBertDataset(Dataset):
    ...
    def __init__(self, path, ds_from=None, ds_to=None, should_include_text=False):
        ...
        if should_include_text:
            ...
            self.columns = [self.MASKED_INDICES_COLUMN, self.TARGET_COLUMN, self.TOKEN_MASK_COLUMN, self.NSP_TARGET_COLUMN]
    ...
    def _create_item(self, first: typing.List[str], second: typing.List[str], target: int=1):
        ...
        updated_first, first_mask = self._preprocessing_sentence(first.copy())
        updated_second, second_mask = self._preprocessing_sentence(second.copy())
        ...
        first, _ = self._preprocessing_sentence(first.copy(), should_mask=False)
        second, _ = self._preprocessing_sentence(second.copy(), should_mask=False)
    ...
    def _preprocessing_sentence(self, sentence: typing.List[str], should_mask: bool = True):
        inverse_token_mask = None
        if should_mask:
            sentence, inverse_token_mask = self._mask_sentence(sentence)
        # print("inverse_token_mask", inverse_token_mask, sentence)
...
if __name__ == '__main__':
    ...
    ds = IMDBBertDataset(BASE_DIR.joinpath('/Users/luandaoduy/Coding/reimplementAITech/bert/data/imdb.csv'), ds_from=0, ds_to=50000,
